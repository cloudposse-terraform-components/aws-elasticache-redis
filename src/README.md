...
| <a name="input_num_shards"></a> [num\_shards](#input\_num\_shards) | Default number of shards (node groups) for Redis clusters. Value > 0 enables cluster mode. It can be overridden per cluster in redis\_clusters. | `number` | `0` | no |
...
| <a name="input_replicas_per_shard"></a> [replicas\_per\_shard](#input\_replicas\_per\_shard) | Default number of replica nodes per shard. Valid values are 0 to 5. It can be overridden per cluster in redis\_clusters. | `number` | `0` | no |
